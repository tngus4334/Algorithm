...
# 방문저장여부 visited를 만들어야하는데
# 공이 2개? 어떻게 해야 방문하는지 아닌지를 알수 있을까?
# 4차원 배열을 사용하면 된다
#              n               m                 n                 m
# visited[red_marble_row][red_marble_col][blue_marble_row][blue_marble_col]
# 3 <= x <= 10
def move_until_wall_or_hole(r, c, diff_r, diff_c, game_map):  # game_map에서 이게 벽인지 구멍인지 알아봐야 하기 때문
    move_count = 0
    while game_map[r + diff_r][c + diff_c] != "#" and game_map != "O":
        r += diff_r
        c += diff_c
        move_count += 1
...
def is_available_to_take_out_only_red_marble(game_map):
    ...
    # 탐색을 10번까지만 할 수 있다 !
    queue.append((red_row, red_col, blue_row, blue_col, 1))  # 현재탐색횟수 1번
